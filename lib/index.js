...
let buildFn;
export const viteIncrementalBuild = ({ config, bundleName = 'bundle', watcherIgnoredFiles, beforeBuildCallback, }) => {
    buildFn = () => {
...
export const patchConfig = (config, { ignoreWarnings = false } = {}) => {
    if (config.root === undefined || !config.root.startsWith('./') || config.root.endsWith('/')) {
        console.log('\x1b[31m%s\x1b[0m', `expected to find 'root' in vite config and for 'root' to start with "./" and not to end with "/"`);
    ...
    config.plugins.unshift({
        ...
        options(options) {
            ...
            if (watcherModifiedFile) {
                ...
                const findMatching = (item) => path.resolve(item[1]) === path.resolve(config.root + '/' + watcherModifiedFile);
...
const buildBundle = async (bundleName, config, beforeBuildCallback) => {
    if (running)
        return;
    running = true;
    beforeBuildCallback?.();
    ...
    console.log('\x1b[90m%s\x1b[0m', 'press r to trigger a full rebuild (if you changed branch or suspect that files are missing)');
    setTimeout(() => {
